    ...
    <header class="ifc-header">
      <div class="header-brand">
        ...
        <div>
          ...
          <p class="subtitle">
            Open BIM · IFC 4 · Node-based placement · Three.js
          </p>
        </div>
      </div>
    ...
    <div class="workspace">
      ...
      <aside class="sidebar">
        ...
        <section class="panel" :class="{ disabled: !model }">
          <h2 class="panel-title">
            <span class="panel-icon">02</span> Nuovo Elemento
          </h2>
          ...
          <template v-if="isLinear(newElement.type)">
            <div class="node-block">
              <div class="node-block-title">
                <span class="node-dot start"></span> Nodo iniziale
              ...
              <div class="coords-grid">
                <div class="field">
                  <label>X (m)</label
                  ><input
                    v-model.number="newElement.n1.x"
                    type="number"
                    step="0.5"
                  />
                </div>
                <div class="field">
                  <label>Y (m)</label
                  ><input
                    v-model.number="newElement.n1.y"
                    type="number"
                    step="0.5"
                  />
                </div>
                <div class="field">
                  <label>Z (m)</label
                  ><input
                    v-model.number="newElement.n1.z"
                    type="number"
                    step="0.5"
                  />
                </div>
            ...
            <div class="node-block">
              <div class="node-block-title">
                <span class="node-dot end"></span> Nodo finale
              ...
              <div class="coords-grid">
                <div class="field">
                  <label>X (m)</label
                  ><input
                    v-model.number="newElement.n2.x"
                    type="number"
                    step="0.5"
                  />
                </div>
                <div class="field">
                  <label>Y (m)</label
                  ><input
                    v-model.number="newElement.n2.y"
                    type="number"
                    step="0.5"
                  />
                </div>
                <div class="field">
                  <label>Z (m)</label
                  ><input
                    v-model.number="newElement.n2.z"
                    type="number"
                    step="0.5"
                  />
                </div>
            ...
            <div class="derived-info">
              <div class="derived-row">
                ...
                <span class="derived-val"
                  >{{
                    nodeLength(newElement.n1, newElement.n2).toFixed(3)
                  }}
                  m</span
                >
              </div>
        ...
        >
          ...
          <template v-if="isLinear(elements[selectedIdx].type)">
              ...
              <div class="coords-grid">
                <div class="field">
                  ...
                  <input
                    type="number"
                    step="0.5"
                    :value="elements[selectedIdx].n1.x"
                    @change="
                      updateNode(
                        selectedIdx,
                        'n1',
                ...
                <div class="field">
                  ...
                  <input
                    type="number"
                    step="0.5"
                    :value="elements[selectedIdx].n1.y"
                    @change="
                      updateNode(
                        selectedIdx,
                        'n1',
                ...
                <div class="field">
                  ...
                  <input
                    type="number"
                    step="0.5"
                    :value="elements[selectedIdx].n1.z"
                    @change="
                      updateNode(
                        selectedIdx,
                        'n1',
              ...
              <div class="coords-grid">
                <div class="field">
                  ...
                  <input
                    type="number"
                    step="0.5"
                    :value="elements[selectedIdx].n2.x"
                    @change="
                      updateNode(
                        selectedIdx,
                        'n2',
                ...
                <div class="field">
                  ...
                  <input
                    type="number"
                    step="0.5"
                    :value="elements[selectedIdx].n2.y"
                    @change="
                      updateNode(
                        selectedIdx,
                        'n2',
                ...
                <div class="field">
                  ...
                  <input
                    type="number"
                    step="0.5"
                    :value="elements[selectedIdx].n2.z"
                    @change="
                      updateNode(
                        selectedIdx,
                        'n2',
            ...
            <div class="derived-info">
              <div class="derived-row">
                <span class="derived-lbl">Lunghezza</span>
                <span class="derived-val accent"
                  >{{
                    nodeLength(
                      elements[selectedIdx].n1,
                      elements[selectedIdx].n2,
                    ).toFixed(3)
                  }}
                  m</span
                >
      ...
      <main class="main-area">
        ...
        <div class="welcome-state" v-if="!model">
          ...
          <p>
            Inizia creando un modello. Gli elementi lineari (travi, pilastri,
            pali…) vengono definiti tramite <strong>nodo iniziale N1</strong> e
            <strong>nodo finale N2</strong> con coordinate 3D. La geometria e la
            rotazione IFC sono calcolate automaticamente.
          </p>
        </div>
        ...
        <div class="viewer-wrap" v-show="model">
          ...
          <div class="n1-picked" v-if="placingMode && pendingN1">
            N1 fissato: ({{ pendingN1.x.toFixed(2) }},
            {{ pendingN1.y.toFixed(2) }}, {{ pendingN1.z.toFixed(2) }}) · ora
            clicca per N2
          </div>
        ...
        <div class="elements-section" v-if="model">
          ...
          <div class="empty-state" v-if="elements.length === 0">
            ...
            <p>
              Nessun elemento. Aggiungine uno dal pannello laterale o con
              "Posiziona nel viewer".
            </p>
          </div>
              ...
              <div class="el-info">
                ...
                <div class="el-meta" v-if="isLinear(el.type)">
                  N1 ({{ el.n1.x }}, {{ el.n1.y }}, {{ el.n1.z }}) → N2 ({{
                    ...
                    nodeLength(el.n1, el.n2).toFixed(2)
                  }}m · {{ el.width }}×{{ el.height }}
                </div>
                <div class="el-meta" v-else>
                  ({{ el.n1.x }}, {{ el.n1.y }}, {{ el.n1.z }}) ·
                  {{ el.length }}×{{ el.width }}×{{ el.height }} m
                </div>
              </div>
...
function onTypeChange() {
  newElement.value = {
    ...makeBlankElement(newElement.value.type),
...
function fitAll() {
  ...
  for (const el of elements.value) {
    const pts = isLinear(el.type)
      ? [el.n1, el.n2]
      : [el.n1, { x: el.n1.x + el.length, y: el.n1.y + el.width, z: el.n1.z }];
    for (const p of pts) {
...
function _handlePlace() {
  ...
  if (isLinear(newElement.value.type)) {
    ...
    addBimElement({
      ...newElement.value,
      n1: { x: snapped.x, y: snapped.y, z: snapped.z },
    });
  }
}
...
function _handleSelect() {
  ...
  const allMeshes = [];
  meshMap.forEach((v) => {
    allMeshes.push(v.mesh);
    if (v.nodeSpheres) allMeshes.push(...v.nodeSpheres);
  });
...
function createModel() {
  model.value = {
    ...
    organization: organization.value || "Org",
...
.log-entry {
  ...
  gap: 8px;
  padding: 4px 10px;
  border-bottom: 1px solid #1a1f3055;
  font-size: 9px;
}
.log-time {
  color: #2a3850;
